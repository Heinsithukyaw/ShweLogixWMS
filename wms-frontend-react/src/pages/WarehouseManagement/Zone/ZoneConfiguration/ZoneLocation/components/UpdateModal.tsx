...
import { updateZoneLocation } from '../services/zoneLocationApi'
...
const ZoneLocationUpdateModal:React.FC<Props> = ({isUpdateOpen,handleCloseModal,zoneId}) => {
   const dispatch = useAppDispatch()
   const [isLoading, setIsLoading] = useState<any>(false)
   const [errors, setErrors] = useState<Errors>({})
   const zoneLists = useAppSelector((state: RootState) => state.zone?.content)
   const zoneLocation = useAppSelector((state: RootState) => state.mLocation?.zoneData)
   ...
   const { showToast } = provideUtility()
     ...
     useEffect(() => {
        ...
        setUpdateFormData({
            id: zoneLocation.id,
            location_code: zoneLocation.location_code,
            location_name: zoneLocation.location_name,
            location_type: zoneLocation.location_type,
            zone_id: zoneLocation.zone_id,
            aisle: zoneLocation.aisle,
            row: zoneLocation.row,
            level: zoneLocation.level,
            bin: zoneLocation.bin,
            capacity: zoneLocation.capacity,
            capacity_unit: zoneLocation.capacity_unit,
            restrictions: zoneLocation.restrictions,
            bar_code: zoneLocation.bar_code,
            description: zoneLocation.description,
            utilization:zoneLocation.utilization,
            status: zoneLocation.status_value,
        })
        }
    },[zoneLocation])
   ...
   const handleChange = (field: string) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    const handleUpdate = async () => {
        setIsLoading(true)
        const result = (await dispatch(updateZoneLocation(updateFormData, zoneLocation?.id,zoneId))) as any
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <IOSSlider
                ...
                value={updateFormData.utilization}
                onChange={(event, newValue) =>
                  setUpdateFormData({ ...updateFormData, utilization: Number(newValue) })
                }
...
export default ZoneLocationUpdateModal