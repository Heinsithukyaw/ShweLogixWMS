...
import { updatePutAwayTask } from '../services/putAwayTaskApi'
...
const PutAwayTaskUpdateModal:React.FC<Props> = ({isUpdateOpen,handleCloseModal}) => {
   ...
   useEffect(() => {
    if(task){
        setUpdateFormData({
          id: task.id,
          put_away_task_code: task.put_away_task_code,
          inbound_shipment_detail_id: task.inbound_shipment_detail_id,
   ...
   const handleChange = (field: string) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    const handleUpdate = async () => {
      setIsLoading(true)
      const result = (await dispatch(updatePutAwayTask(updateFormData, task?.id))) as any
      ...
      if (result?.error.status === 422) {
        ...
        handleCloseModal()
        showToast('', 'Updated Putaway Task Successfully', 'top-right', 'success')
      } else {
        showToast('Error', 'Failed to update Putaway Task', 'top-right', 'error')
      }
    }
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <Input
                type="text"
                value={updateFormData.put_away_task_code}
...
export default PutAwayTaskUpdateModal